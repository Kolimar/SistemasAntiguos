...
requestPass(email) {
       let respuesta = this.http.post(global.urlApi+'password', email)
                        .map((response: Response) => {
                                response.json()
                            });
        return respuesta;
...
changePass(email: string, password: string,token) {
       let respuesta = this.http.post(global.urlApi+'password/reset/'+ token +'?email='+email+'&password='+password,email)
                        .map((response: Response) => {
                                response.json()
                            });
        return respuesta;
